...
const sidebarLinksByRole = {
    ...
    accountant: [
        ...
        { name: "Structure des Frais", url: "/static/dist/html/finance/fees.html", icon: "settings" },
        { name: "Transactions", url: "/static/dist/html/finance/transactions.html", icon: "list" },
        { name: "Rapports", url: "/static/dist/html/finance/reports.html", icon: "file-text" },
        { name: "Perception", url: "/static/dist/html/finance/student_payments.html", icon: "wallet" }
    
    ],

    cashier: [
        { name: "Ma Caisse", url: "/static/dist/html/finance/cashier_desk.html", icon: "wallet" },
        { name: "Perception", url: "/static/dist/html/finance/collect_fee.html", icon: "plus-circle" },
    ]
};
...
function renderSidebar(roleKey, name, email, pic) {
    ...
    const roleDisplayName = roleKey.charAt(0).toUpperCase() + roleKey.slice(1).replace('_', ' ');
        ...
        <aside id="sidebar-container" 
            ...
            <div class="h-20 flex items-center justify-between px-6 border-b dark:border-slate-700/50 bg-white/80 dark:bg-slate-900/80 backdrop-blur-sm">
                <div class="flex items-center gap-3">
                    <div class="relative">
                        <img src="/static/src/img/Transpa-Logo.PNG" alt="SYBEM" class="h-10 w-10 object-contain drop-shadow-lg"/>
                        <div class="absolute inset-0 bg-indigo-500/10 rounded-full blur-sm"></div>
                    </div>
                    <div>
                        <p class="text-lg font-bold bg-gradient-to-r from-indigo-600 to-purple-600 bg-clip-text text-transparent">SYBEM</p>
                        <p class="text-[10px] text-slate-500 dark:text-slate-400 font-medium">Education Suite</p>
                    </div>
                </div>
                <button onclick="toggleSidebar()" class="lg:hidden p-2 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg transition-colors group">
            ...
            <div class="px-6 py-5 border-b dark:border-slate-700/50 flex items-center gap-4 group hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors">
                <div class="relative">
                    <img src="${pic || avatarDefault}" class="h-12 w-12 rounded-full border-2 border-white dark:border-slate-700 shadow-lg object-cover"/>
                ...
                <div class="overflow-hidden flex-1">
                    ...
                    <span class="inline-block mt-1 px-2 py-0.5 bg-indigo-100 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-300 text-[10px] font-bold uppercase rounded-full">
                        ${roleDisplayName}
                    </span>
                </div>
            </div>
                    ...
                    <a href="${link.url}" 
                       class="flex items-center gap-3 px-4 py-3 text-slate-700 dark:text-slate-300 
                        ...
                        <div class="relative z-10 flex items-center gap-3">
                            <i data-lucide="${link.icon}" class="w-5 h-5 transition-transform group-hover:scale-110"></i>
